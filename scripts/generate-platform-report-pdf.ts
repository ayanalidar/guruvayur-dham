...
Generates an in-depth PDF document covering all platform features, SEO reach,
...
from reportlab.platypus.doctemplate import SimpleDocTemplate as SDT
import hashlib
...
pdfmetrics.registerFont(TTFont('NotoSansSC', f'{FONT_DIR}/chinese/NotoSansSC-Regular.ttf'))
pdfmetrics.registerFont(TTFont('NotoSansSC-Bold', f'{FONT_DIR}/chinese/NotoSansSC-Bold.ttf'))
registerFontFamily('NotoSerifSC', normal='NotoSerifSC', bold='NotoSerifSC-Bold')
registerFontFamily('NotoSansSC', normal='NotoSansSC', bold='NotoSansSC-Bold')

# ─── Color Palette (champagne/gold/ink theme matching the site) ─────────────
INK = HexColor('#0F0A08')
INK_SOFT = HexColor('#1A1410')
CHAMPAGNE = HexColor('#D4AF37')
CHAMPAGNE_LIGHT = HexColor('#E8D091')
GOLD = HexColor('#C9A227')
IVORY = HexColor('#F5F0E8')
CREAM = HexColor('#FAF6EE')
SAFFRON = HexColor('#FF7733')
...
style_cover_title = ParagraphStyle(
    ...
    textColor=CHAMPAGNE, alignment=TA_CENTER, spaceAfter=12
)
style_cover_subtitle = ParagraphStyle(
    'CoverSubtitle', parent=styles['Normal'],
    fontName='NotoSerifSC', fontSize=16, leading=22,
    textColor=IVORY, alignment=TA_CENTER, spaceAfter=8
)
style_cover_tag = ParagraphStyle(
    'CoverTag', parent=styles['Normal'],
    fontName='NotoSansSC', fontSize=10, leading=14,
    textColor=CHAMPAGNE_LIGHT, alignment=TA_CENTER
)
...
style_h2 = ParagraphStyle(
    ...
    textColor=MAROON, spaceBefore=18, spaceAfter=8, keepWithNext=True
)
...
style_toc_l0 = ParagraphStyle(
    'TOCL0', fontName='NotoSerifSC-Bold', fontSize=12, leading=18,
    textColor=INK, leftIndent=0, spaceBefore=6
)
style_toc_l1 = ParagraphStyle(
    'TOCL1', fontName='NotoSerifSC', fontSize=10.5, leading=15,
    textColor=MUTED, leftIndent=18, spaceBefore=2
)
style_table_header = ParagraphStyle(
    'TableHeader', fontName='NotoSansSC-Bold', fontSize=9, leading=12,
    textColor=white, alignment=TA_CENTER
)
style_table_cell = ParagraphStyle(
    'TableCell', fontName='NotoSerifSC', fontSize=9, leading=13,
...
style_note = ParagraphStyle(
    ...
    fontSize=9.5, leading=14, textColor=MUTED, leftIndent=12, spaceBefore=4
)
...
def bullets(items, style=style_bullet):
    ...
    return ListFlowable(
        ...
        bulletType='bullet', bulletColor=CHAMPAGNE, bulletFontSize=8,
...
def make_table(data, col_widths=None, header_bg=INK):
    ...
    t.setStyle(TableStyle([
        ...
        ('ROWBACKGROUNDS', (0, 1), (-1, -1), [LIGHT_BG, CREAM]),
...
def cover_page(canvas, doc):
    ...
    canvas.rect(0, h - 8, w, 8, fill=1)
    # Gold gradient strip at bottom
    canvas.rect(0, 0, w, 8, fill=1)
    # Decorative Om symbol (large, faint)
    canvas.setFillColor(HexColor('#D4AF37'))
    canvas.setFillAlpha(0.04)
    canvas.setFont('NotoSerifSC', 280)
    canvas.drawCentredString(w / 2, h / 2 - 100, 'OM')
    canvas.setFillAlpha(1)
    # Brand name at top
    canvas.setFillColor(CHAMPAGNE)
    canvas.setFont('NotoSerifSC-Bold', 14)
    canvas.drawCentredString(w / 2, h - 50, 'GURUVAYUR DHAM')
    canvas.setFillColor(CHAMPAGNE_LIGHT)
...
story.append(Paragraph(
    'Guruvayur Dham is a comprehensive digital platform for a luxury pilgrim hotel in Mathura, Uttar Pradesh, '
    'located opposite Mata Pathwari Mandir in Natwar Nagar, Dholi Pyau. Built on Next.js 16 with a PostgreSQL '
    'database, the platform serves as a complete hotel management system, content management system (CMS), '
    'pilgrim assistant, and SEO-driven marketing engine all in one.',
    style_body
))
story.append(Paragraph(
    'This document provides an in-depth analysis of every feature built into the platform, the SEO reach '
    'across 23+ pages, the technology stack, deployment infrastructure, and operational capabilities. The '
    'platform has been designed with a pilgrim-first approach, understanding that devotees visiting Mathura '
    'need not just a room, but a spiritual companion that coordinates darshan timings, pooja bookings, festival '
    'planning, and temple visits.',
    style_body
))
...
story.append(Paragraph(
    'The platform is built on a modern, production-grade technology stack designed for performance, '
...
story.append(Paragraph(
    'The platform features 23+ pages organized into three tiers: core pages (the main hotel website), '
    'SEO landing pages (designed to capture search traffic for high-intent pilgrim queries), and '
    'administrative pages (CMS, dashboard, settings). Every page is connected via a hash-based router '
    'and fully integrated with the CMS for instant content updates.',
    style_body
))

story.append(heading('2.1 Core Pages (10)', style_h2, 1))
story.append(Paragraph(
    'These are the primary pages of the hotel website. Each is fully CMS-driven, meaning all text, '
    'images, and data can be edited through the admin dashboard without touching code.',
    style_body
))
story.append(make_table([
    ['Page', 'Route', 'Purpose'],
...
story.append(Paragraph(
    'These are purpose-built pages designed to capture high-intent search traffic from pilgrims planning '
    'their Mathura visit. Each page has 800+ words of SEO-optimized content, JSON-LD structured data, '
    'FAQ accordion, and a booking CTA. The combined monthly search reach is estimated at 750,000+ searches.',
    style_body
))
...
story.append(Paragraph(
    'These pages target bottom-of-funnel searches: pilgrims who have decided which temple to visit and '
    'now need a room nearby. Conversion rates on these pages are very high.',
    style_body
))
story.append(make_table([
    ['Page', 'Slug', 'Est. Monthly Searches', 'JSON-LD Schema'],
...
story.append(Paragraph(
    'Evergreen content pages targeting pilgrims searching for temple timings. These position the hotel '
    'as the local authority and naturally drive bookings.',
    style_body
))
story.append(make_table([
    ['Page', 'Slug', 'Est. Monthly Searches', 'JSON-LD Schema'],
...
# ════════════════════════════════════════════════════════════════════════════
# CHAPTER 3: CONTENT MANAGEMENT SYSTEM (CMS)
# ════════════════════════════════════════════════════════════════════════════
story.append(heading('3. Content Management System (CMS)', style_h1, 0))

story.append(Paragraph(
    'The platform includes a full-featured CMS that allows non-technical staff to edit every piece of text '
    'and every image on the website without touching code. The CMS is organized into two editors: a '
    'content block editor (for section text like headlines, subtitles, contact info) and a structured data '
    'editor (for lists like rooms, events, FAQs, testimonials, poojas, blog posts, and SEO pages).',
    style_body
))

story.append(heading('3.1 Content Block Editor (/admin/content)', style_h2, 1))
story.append(Paragraph(
    'The content block editor manages 70+ editable text and image fields organized into 16 categories. '
    'Changes go live instantly after saving. The editor supports search, category filtering, and unsaved '
    'change indicators.',
    style_body
))
story.append(make_table([
...
story.append(Paragraph(
    'The structured data editor manages list-based content. It has 11 tabs, each with full CRUD '
    '(create, read, update, delete) operations. All changes go live instantly.',
    style_body
))
story.append(make_table([
...
story.append(Paragraph(
    'The CMS includes an integrated image upload system that works across all editors. When a user uploads '
    'an image, it is stored via Vercel Blob (cloud storage, persistent across deploys) if the '
    'BLOB_READ_WRITE_TOKEN environment variable is set. Otherwise, it falls back to local filesystem '
    'storage at /public/uploads/ (persistent on VPS, ephemeral on Vercel serverless).',
    style_body
))
story.append(bullets([
...
# ════════════════════════════════════════════════════════════════════════════
# CHAPTER 4: BOOKING & REVENUE
# ════════════════════════════════════════════════════════════════════════════
story.append(heading('4. Booking &amp; Revenue System', style_h1, 0))

story.append(Paragraph(
    'The platform includes a complete hotel booking system with dynamic pricing, coupon support, '
    'early-bird discounts, multi-channel inventory sync, and automated post-stay follow-up. The booking '
    'flow is designed to be completed in under 30 seconds via WhatsApp, matching the pilgrim-friendly '
...
story.append(Paragraph(
    'The guest booking page (/#/book) implements a 4-step wizard:',
    style_body
))
story.append(make_table([
...
story.append(Paragraph(
    'The pricing engine calculates room rates based on multiple factors, ensuring optimal revenue during '
    'peak seasons while remaining competitive during off-peak periods:',
    style_body
))
story.append(bullets([
...
story.append(Paragraph(
    'When a booking is made (direct, walk-in, or from an OTA), the platform automatically broadcasts a '
    'BLOCK signal to all other connected channel partners (Booking.com, MakeMyTrip, Goibibo, Agoda). This '
    'prevents double-bookings across channels. Each channel partner has a dedicated webhook endpoint:',
    style_body
))
story.append(make_table([
...
story.append(Paragraph(
    'Each webhook endpoint also has a GET health check that returns the channel connection status, last '
    'sync time, total bookings, and setup instructions for the partner dashboard. The sync logic is '
    'centralized in /lib/channel-sync.ts for reuse across all booking entry points.',
    style_body
))
...
# ════════════════════════════════════════════════════════════════════════════
# CHAPTER 5: WHATSAPP CHATBOT
# ════════════════════════════════════════════════════════════════════════════
story.append(heading('5. WhatsApp Chatbot', style_h1, 0))

story.append(Paragraph(
    'The platform features a floating WhatsApp chat widget on every page (except the login page). The '
    'bot uses intent recognition for common pilgrim queries and falls back to AI (Groq LLaMA 3.3) for '
    'general questions. It also includes a production-ready webhook for real WhatsApp Business API '
    'integration.',
    style_body
))
...
story.append(Paragraph(
    'The bot recognizes the following intents without AI (instant response, zero cost):',
    style_body
))
story.append(make_table([
...
story.append(Paragraph(
    'For real WhatsApp messaging (not just the website widget), the platform includes a production-ready '
    'webhook endpoint at /api/whatsapp/webhook. This handles Meta WhatsApp Business API webhook '
    'verification (GET) and incoming message processing (POST).',
    style_body
))
story.append(bullets([
...
# ════════════════════════════════════════════════════════════════════════════
# CHAPTER 6: POST-STAY REVIEW FUNNEL
# ════════════════════════════════════════════════════════════════════════════
story.append(heading('6. Post-Stay Review Funnel', style_h1, 0))

story.append(Paragraph(
    'The post-stay review funnel is an automated system that sends a Google Reviews request to guests '
    '2 hours after they check out. This is the single highest-ROI feature for improving the hotel Google '
    'ranking, as more positive reviews directly translate to higher search visibility and more direct '
    'bookings (reducing OTA commission costs).',
    style_body
))
...
# ════════════════════════════════════════════════════════════════════════════
# CHAPTER 7: SEO & STRUCTURED DATA
# ════════════════════════════════════════════════════════════════════════════
story.append(heading('7. SEO &amp; Structured Data', style_h1, 0))

story.append(Paragraph(
    'The platform is built with SEO as a first-class priority. Every page has optimized meta tags, '
    'JSON-LD structured data, semantic HTML, and fast load times. The 13 SEO landing pages alone '
    'target an estimated 750,000+ monthly searches across festivals, hotel-near queries, and darshan '
    'timing guides.',
    style_body
))
...
story.append(Paragraph(
    'Google uses structured data to understand page content and display rich results (star ratings, '
    'event dates, FAQ accordions, etc.). The platform injects the following schemas:',
    style_body
))
story.append(make_table([
...
story.append(Paragraph(
    'The platform generates a dynamic sitemap.xml at /sitemap.xml that includes:',
    style_body
))
story.append(bullets([
...
story.append(Paragraph(
    'A dynamic robots.txt at /robots.txt allows all major bots (Googlebot, Bingbot, Twitterbot, Facebook) '
    'to crawl public content while blocking admin, API, booking, login, kitchen, CMS, and settings routes. '
...
story.append(Paragraph(
    'The platform includes a built-in SEO audit tool at /admin/settings that performs real checks on the '
    'live site. Unlike the previous version (which awarded fake 100/100 scores), the current audit:',
    style_body
))
story.append(bullets([
...
# ════════════════════════════════════════════════════════════════════════════
# CHAPTER 8: MULTILINGUAL SUPPORT
# ════════════════════════════════════════════════════════════════════════════
story.append(heading('8. Multilingual Support (i18n)', style_h1, 0))

story.append(Paragraph(
    'The platform supports 5 languages: English, Hindi, Marathi, Gujarati, and Malayalam. A language '
    'selector in the navbar allows instant switching, and all page content (not just navigation labels) '
    'translates automatically. The system uses a three-layer translation approach: CMS admin-curated '
    'translations, built-in translation file, and hardcoded fallbacks.',
    style_body
))

story.append(heading('8.1 How Language Switching Works', style_h2, 1))
story.append(Paragraph(
    'The useContent() hook integrates with the i18n system. When resolving a content block, it checks '
    'in order:',
    style_body
))
story.append(bullets([
...
story.append(Paragraph(
    'The platform is designed for dual deployment: Vercel (current production) and any VPS via Docker '
    '(future migration). Both paths are fully supported with zero code changes. The VPS setup includes '
    'auto-HTTPS via Caddy, PostgreSQL in a container, and a dedicated cron container for the review funnel.',
    style_body
))
...
story.append(Paragraph(
    'The platform is fully VPS-ready with a complete Docker setup. A single deploy.sh script handles '
    'all operations: build, deploy, migrate, seed, backup, restore, logs, and status.',
    style_body
))
story.append(make_table([
...
# ════════════════════════════════════════════════════════════════════════════
# CHAPTER 10: ADMIN & OPERATIONS
# ════════════════════════════════════════════════════════════════════════════
story.append(heading('10. Admin &amp; Operations Dashboard', style_h1, 0))

story.append(Paragraph(
    'The platform includes a comprehensive admin dashboard for hotel staff. Accessible at /#/admin, it '
    'provides real-time visibility into bookings, availability, channel sync, reviews, housekeeping, '
    'kitchen orders, and more. Role-based access control supports GUEST, STAFF, and MANAGER roles.',
    style_body
))
...
# ════════════════════════════════════════════════════════════════════════════
# CHAPTER 11: PWA & PERFORMANCE
# ════════════════════════════════════════════════════════════════════════════
story.append(heading('11. PWA &amp; Performance', style_h1, 0))

story.append(Paragraph(
    'The platform is a Progressive Web App (PWA) with offline support, push notifications, and '
    'add-to-home-screen capability. Performance is optimized for fast load times on mobile networks, '
    'which is critical for pilgrim users in areas with poor connectivity.',
    style_body
))
...
story.append(Paragraph(
    'The platform implements multiple layers of security to protect guest data, prevent abuse, and '
    'ensure reliable operation. All sensitive operations are server-side, and authentication is required '
    'for all admin and booking endpoints.',
    style_body
))
...
# ════════════════════════════════════════════════════════════════════════════
# CHAPTER 13: FUTURE ROADMAP
# ════════════════════════════════════════════════════════════════════════════
story.append(heading('13. Future Roadmap', style_h1, 0))

story.append(Paragraph(
    'The platform is designed for continuous growth. The following features are recommended for the '
    'next phase of development, ranked by expected impact:',
    style_body
))
...
story.append(Paragraph(
    'The Guruvayur Dham platform is a comprehensive, production-ready digital solution for a pilgrim '
    'hotel. It combines a beautiful, fast website with a powerful CMS, intelligent WhatsApp chatbot, '
    'automated review funnel, multi-channel booking sync, and 13 SEO-optimized landing pages targeting '
    '750,000+ monthly searches.',
    style_body
))

story.append(Paragraph(
    'The platform is deployed on Vercel with automatic deploys from GitHub, and is also fully VPS-ready '
    'with a complete Docker setup. All content is editable through the CMS without code changes, and '
    'the site supports 5 languages for pilgrims from across India.',
    style_body
))
...
story.append(Paragraph(
    '<i>This document was generated on January 2026 and reflects the current state of the platform. '
    'For the latest features and updates, visit the GitHub repository at '
    'github.com/ayanalidar/guruvayur-dham.</i>',
    style_note
))
...
doc = TocDocTemplate(
    ...
    topMargin=25 * mm,
    bottomMargin=20 * mm,
    title='Guruvayur Dham Platform Report',
    author='Guruvayur Dham',
    subject='Platform Features, SEO Reach, and Technical Documentation',
    creator='Z.ai',
)
